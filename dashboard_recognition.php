...
// Load user's saved default data from database
$user_defaults = [];
$stmt = $conn->prepare("SELECT name, department FROM user_defaults WHERE user_id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_defaults'])) {
    ...
    $stmt = $conn->prepare("SELECT user_id FROM user_defaults WHERE user_id = ?");
    $stmt->bind_param("s", $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt->close();
        $stmt = $conn->prepare("UPDATE user_defaults SET name = ?, department = ?, updated_at = NOW() WHERE user_id = ?");
        $stmt->bind_param("sss", $name, $department, $user_id);
    } else {
        // Insert new defaults
        $stmt->close();
        $stmt = $conn->prepare("INSERT INTO user_defaults (user_id, name, department) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $user_id, $name, $department);
    }
    
    if ($stmt->execute()) {
        $success_message = "Default data saved successfully! Your information will be auto-filled next time.";
        $user_defaults['name'] = $name;
        $user_defaults['department'] = $department;
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_recognition'])) {
    ...
    // Save to database only if no errors
    if (empty($error_message)) {
        $stmt = $conn->prepare("INSERT INTO recognitions (user_id, submitted_by, name, department, category, program_name, recognition_type, date, level, document_path, status, submitted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW())");
        $stmt->bind_param("ssssssssss", $user_id, $full_name, $name, $department, $category, $program_name, $recognition_type, $date, $level, $file_path);
        ...
        if ($stmt->execute()) {
            ...
            // Log the activity
            $activity_description = "Submitted recognition: $program_name";
            $log_stmt = $conn->prepare("INSERT INTO activity_logs (user_id, activity_type, description) VALUES (?, 'submission', ?)");
            $log_stmt->bind_param("ss", $user_id, $activity_description);
            $log_stmt->execute();
            $log_stmt->close();
        } else {
