...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($user_id) || empty($full_name) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        $stmt = $conn->prepare("SELECT user_id FROM registration WHERE user_id = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            if ($result->num_rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO registration (user_id, full_name, email, password, registered_at) VALUES (?, ?, ?, ?, NOW())");
